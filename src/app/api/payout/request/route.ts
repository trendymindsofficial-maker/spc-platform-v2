...
export async function POST(request: Request) {
  try {
    ...
    // Support the current student dashboard field name
    // and keep backward compatibility with accountName.
    ...
    if (!["upi", "bank"].includes(method)) {
      return NextResponse.json(
        {
          success: false,
          error: "Choose UPI or Bank Account.",
        },
        { status: 400 }
      );
    }
    ...
    await db.runTransaction(async (tx) => {
      ...
      tx.set(payoutRef, {
        ...
        // Store the canonical field used by admin payout UI.
