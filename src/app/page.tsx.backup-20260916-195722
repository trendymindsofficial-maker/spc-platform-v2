...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-white pb-20 text-[#07111f] sm:pb-0">
      ...
      {/* HOW IT WORKS */}
      <section id="how-it-works" className="relative overflow-hidden bg-[#f7faff] px-4 py-14 sm:px-6 sm:py-16">
        <div className="absolute -left-24 top-10 h-52 w-52 rounded-full bg-yellow-200/25 blur-3xl" />
        <div className="absolute -right-24 bottom-0 h-60 w-60 rounded-full bg-purple-200/25 blur-3xl" />

        <div className="relative mx-auto max-w-[1250px]">
          <div className="text-center">
            <p className="text-[10px] font-black uppercase tracking-[.24em] text-[#1557d6]">SIMPLE • REWARDING • FOR STUDENTS</p>
            <h2 className="mt-2 text-3xl font-black tracking-tight text-[#07111f] sm:text-5xl">
              How <span className="text-[#1557d6]">SBC</span> Works
            </h2>
            <p className="mx-auto mt-3 max-w-xl text-sm font-semibold text-slate-500 sm:text-base">
              Save More. Earn More. Grow Together.
            </p>
          </div>
            ...
            {[
              {
                ...
                icon: "🏷️",
                text: "Explore offers from SBC partner businesses and enjoy exclusive student discounts with your SBC Card.",
                pill: "Food • Shopping • Travel • Education",
              },
              {
                number: "02",
                title: "Use & Earn Points",
                accent: "text-emerald-600",
                badge: "bg-emerald-100 text-emerald-700",
                card: "border-emerald-100 bg-gradient-to-br from-white via-white to-emerald-50",
                icon: "🎁",
                text: "Use your eligible SBC benefits, earn reward points and collect points towards exciting gifts.",
                pill: "Collect Points • Unlock Gifts",
              },
              {
                number: "03",
                title: "Refer & Earn",
                ...
                icon: "📣",
                text: "Invite your friends to join SBC. When successful referrals complete their membership, you earn referral rewards.",
                pill: "More Friends • More Rewards",
              },
            ].map((step) => (
              <div key={step.number} className={`group relative overflow-hidden rounded-[2rem] border p-6 shadow-[0_14px_45px_rgba(15,23,42,0.07)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(15,23,42,0.11)] sm:p-7 ${step.card}`}>
                <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-white/70" />
                <div className="relative flex items-start justify-between gap-4">
                  <span className={`flex h-12 w-12 items-center justify-center rounded-2xl text-sm font-black ${step.badge}`}>
                    {step.number}
                  </span>
                  <span className="flex h-16 w-16 items-center justify-center rounded-[1.4rem] bg-white text-4xl shadow-sm">
                    {step.icon}
                  </span>
                </div>

                <div className="relative mt-6">
                  <h3 className={`text-2xl font-black ${step.accent}`}>{step.title}</h3>
                  <p className="mt-3 min-h-[72px] text-sm leading-6 text-slate-600">{step.text}</p>
                </div>

                <div className="relative mt-5 rounded-2xl bg-white/75 px-4 py-3 text-xs font-black text-slate-700 shadow-sm">
                  {step.pill}
                </div>
              </div>
            ))}
          ...
          <div className="mx-auto mt-7 flex max-w-2xl items-center justify-center rounded-full border border-blue-100 bg-white/80 px-5 py-3 text-center text-xs font-black text-[#07111f] shadow-sm sm:text-sm">
            ⚡ One Card. More Discounts. More Points. More Gifts.
          </div>
        </div>
      </section>
